...
export async function POST(req: NextRequest) {
  try {
    ...
    const key = `avatars/agents/${agentId}/profile.${ext}`;
...
export async function DELETE(_req: NextRequest) {
  try {
    ...
    if (agent.avatarUrl) {
      const key = agent.avatarUrl.replace(`${R2_PUBLIC_URL}/`, "");
      try {
        await r2.send(
        ...
        console.error("[settings/avatar] R2 delete failed for key", key, err);
      }
    }
    ...
    console.error("[settings/avatar] DELETE error", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
